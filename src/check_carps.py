...
known_hwlbs = {
    key
    for iface_dict in carp_settings.ifaces.values()
    for key in iface_dict.keys()
}
...
result_txt = 'my role is {:s}; '.format(my_role)
result_code = 0

p = subprocess.Popen(['/sbin/ifconfig', '-l'], stdout=subprocess.PIPE,
                     stderr=subprocess.PIPE)
ifaces, err = p.communicate()
ifaces = ifaces.split()

carps9 = []
for carp_k, carp_v in iter(carp_settings.ifaces):
    for hwlb_k, hwlb_v in carp_v.items():
        carps9.append('carp' + hwlb_k.split('lb')[1] + carp_k)

for ifname in carp_settings.ifaces.keys() + carps9:
    # Read interface configuration:
    p = subprocess.Popen(['/sbin/ifconfig', ifname], stdout=subprocess.PIPE,
                         stderr=subprocess.PIPE)
    ifconfig, err = p.communicate()

    for line in ifconfig.split(b'\n'):
        try:
            # Find carp lines, the look like this:
            # carp: MASTER vhid 133 advbase 1 advskew 50
            ifconfig_match = re.match(
                '.*carp: ([A-Z]+) vhid ([0-9]+) advbase.*', line)
        ...
        if (
            my_role == 'BACKUP' and status != 'BACKUP' or
            my_role == 'MASTER' and status != 'MASTER'
        ):
            result_code = exit_crit
            result_txt += 'carp {:d} is {:s}; '.format(vhid, status)
...
print("{:s}: {:s}".format(result_dscr, result_txt))
