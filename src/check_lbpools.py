...
def master_status():
    carp_settings = imp.load_source('carp_settings',
                                    '/etc/iglb/carp_settings.py')

    for ifname in carp_settings.ifaces.keys():
        # Read interface configuration:
        p = subprocess.Popen(['/sbin/ifconfig', ifname],
...
def compare_pools(pools, testtool, send):
    ...
    for pool_k, pool_v in pools.items():
        ...
        if pool_v['has_healthchecks']:
            ...
            if optimal_nodes == 1:
                output += (
                    '{}\t{}\t{}\tHas no healthchecks and only 1 node{}'.format(
                        pool_v['nagios_host'],
                        pool_v['nagios_service'],
                        exit_ok,
                        separator
                ...
                output += (
                    '{}\t{}\t{}\tHas no healthchecks but {} nodes{}'.format(
                        pool_v['nagios_host'],
                        pool_v['nagios_service'],
                        exit_warn,
                        optimal_nodes,
